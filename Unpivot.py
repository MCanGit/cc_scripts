...
File = "CO - Master_Schedule - 120423"
...
njoiners.to_csv('//lisfs1003/honey_badger$/Operations - Management/Lisbon Reporting/26. WFM (Reporting)/15. MMT_IW/test_billable.csv', index=False)
...
# -- CF FCST Movements
cf_mov = cf_fcst.query('Forecasted_Change_cf == "Movement"')
df4 = df4.merge(cf_mov[["EID_cf", "Movement_Effective_Date_cf", "To_Slob_cf", "To_Market_cf"]], how='left', left_on=['EID', 'date'], right_on=['EID_cf', 'Movement_Effective_Date_cf'])

def fcst_mov():
    df4['Movement_Effective_Date_cf'] = df4.groupby('EID')['Movement_Effective_Date_cf'].fillna(method='ffill')
    df4['To_Slob_cf'] = df4.groupby('EID')['To_Slob_cf'].fillna(method='ffill')
    df4['To_Market_cf'] = df4.groupby('EID')['To_Market_cf'].fillna(method='ffill')
    df4["market_mmt(cluster)"] = np.where(df4["date"] >= df4['Movement_Effective_Date_cf'], df4["To_Market_cf"], df4["market_mmt(cluster)"])
    df4["SLOB_mmt"] = np.where(df4["date"] >= df4['Movement_Effective_Date_cf'], df4["To_Slob_cf"], df4["SLOB_mmt"])

if not cf_mov.EID_cf.index.empty: fcst_mov()
df4 = df4.drop(columns=["EID_cf", "Movement_Effective_Date_cf", "To_Slob_cf", "To_Market_cf"])
...
df4["ABS_FCST"] = np.where(df4["ABS_FCST"].isnull(), 0, df4["ABS_FCST"])
df4["ABS_FCST"] = df4["ABS_FCST"].astype(int)
